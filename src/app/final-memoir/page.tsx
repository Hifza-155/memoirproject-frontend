...
import {
  ...
  mockMemories,
  mockShortQuotes,
} from "@/features/FinalMemoir/mockData";
...
export default function FinalMemoirPage() {
    ...
    try {
      ...
      if (savedMemoir) {
        ...
        if (obj.subject_name) {
          ...
          setPdfFileName(
            `${obj.subject_name.toLowerCase().replace(/\s+/g, "-")}-story`
          );
        }
    ...
    async function fetchLiveMemoirData() {
      try {
        ...
        if (liveData.memoir) {
          ...
          if (m.subject_born_on) {
            setDob(m.subject_born_on.substring(0, 4));
          }

          if (m.subject_died_on) {
            setDod(m.subject_died_on.substring(0, 4));
          } else if (m.subject_is_living) {
            setDod("Present");
          }
        }
        ...
        if (Array.isArray(memoriesData) && memoriesData.length > 0) {
          ...
          const mapped: MemoryItem[] = memoriesData.map((record) => {
            ...
            const photoAssets =
              record.media_assets?.filter((m) => m.kind === "photo") || [];
            ...
            const images: MemoryImage[] = photoAssets
              .map((asset) => ({
                ...
                caption:
                  asset.caption || record.title || "Archive photo",
              }))
            ...
            images.forEach((img) =>
              photosExtracted.push({
                id: img.id,
                url: img.url,
                caption: img.caption || "Archive photo",
              })
            );
            ...
            const formattedDate = record.occurred_start
              ? new Date(record.occurred_start).toLocaleDateString(
                  "en-US",
                  {
                    year: "numeric",
                    month: "long",
            ...
            const assignedChapterInfo =
              record.chapter_id && chapterMap[record.chapter_id]
                ...
                : {
                    title: "Memoir Reflections",
                    summary: "Stories and preserved moments.",
                  };
  ...
  const activeMemories =
    liveMemories.length > 0 ? liveMemories : mockMemories;

  const activeHeroPhotos =
    livePhotos.length > 0 ? livePhotos : mockHeroPhotos;
  ...
  const formatCommentsToTree = (
    ...
    entities.forEach((entity) => {
      const item: CommentItem = {
        ...
        text: entity.body,
        time: new Date(entity.created_at).toLocaleTimeString([], {
          hour: "2-digit",
          minute: "2-digit",
        }),
        replies: [],
    ...
    entities.forEach((entity) => {
      ...
      if (
        ...
        commentMap.has(entity.parent_comment_id)
      ) {
  ...
  const handlePostComment = async (id: string) => {
    ...
    try {
      ...
      alert(
        "Could not post comment. Please ensure you are logged in and authorized."
      );
    }
  };
  ...
  const handlePostReply = async (
    ...
    try {
      ...
      alert(
        "Could not post reply. Please ensure you are logged in and authorized."
      );
    }
  };
  ...
  const memoryChapterNames = Array.from(
    new Set(activeMemories.map((m) => m.chapter))
  );
  ...
  const uniqueChapters =
    liveChaptersList.length > 0
      ? [
          ...liveChaptersList,
          ...memoryChapterNames.filter(
            (c) => !liveChaptersList.includes(c)
          ),
        ]
      : memoryChapterNames;
          ...
          >
            ...
            {loadingFeed ? (
              ...
              uniqueChapters.map((chapterName, chapterIdx) => {
                const chapterMemories = filteredMemories.filter(
                ...
                const chapterSub =
                  chapterMemories[0].chapterSubtitle;
                    ...
                    {(() => {
                      ...
                      return (
                        <MemoryCard
                          ...
                          mem={combinedMemory}
                          isHighlighted={
                            combinedMemory.reactionsCount > 20
                          }
                          currentReaction={
                            reactions[anchorId] || {
                              count: combinedReactions,
                              reacted: false,
                            }
                          }
                          ...
                          isCommentsOpen={
                            openCommentsId === anchorId
                          }
                          setOpenCommentsId={setOpenCommentsId}
                          commentsList={
                            commentsMap[anchorId] || []
                          }
                          commentInputValue={
                            commentInputs[anchorId] || ""
                          }
              ...
              <div className="mt-16 flex flex-col items-center justify-center opacity-90 pb-8">
                ...
                <div className="mt-8 text-center border-t border-stone-200/60 pt-6 flex flex-col items-center">
                  ...
                  <p className="text-[16px] md:text-[18px] font-serif italic text-stone-600 max-w-lg leading-loose px-4">
                    Dedicated to {subjectName} and preserved for family and
                    loved ones.
                  </p>
                </div>
              </div>
